...
if not ssh:
    ...
    with cols[1]:
        ...
        if not selected_ctrlfldr:
            selected_ctrlfldr = folders_ctrl[0]
        cmd2 = FilesList(ssh, filematch='*.csv', folder=selected_ctrlfldr, button="Get ctrls")
        PageStore().add_to_page("sc2", cmd2)        
        sel_fl2 = st.radio("Choose control", cmd2.files_list, index=0)
